...
export interface IFeedBusinessPages {
  pages: IFeedPage[];
  pageParams?: number[];
}
...
async function fetchBusinessFeeds(
  businessId: string,
  pageNo: number,
): Promise<IFeedPage | undefined> {
  try {
    const offset = (pageNo - 1) * PER_PAGE;
    const url = `${config.BUSINESS_PAGE_API_URL}businessPage/${businessId}/posts?offset=${offset}&limit=${PER_PAGE}`;
...
const useBusinessNewsFeed = (businessId = '', enabled = false) => {
  const cacheKey = [QueryKeys.businessFeed, businessId];
  const listQuery = useInfiniteQuery(
    cacheKey,
    ({pageParam = 1}) => fetchBusinessFeeds(businessId, pageParam),
    {
